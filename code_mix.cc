...
void rejMethode()
{
  ...
  // section efficace
  double sigmaMaxUp = 2.83894e-09;
  double sigmaMaxDown = 7.2e-10;
  // Declarez toutes les variables necessaires pour
  // votre programme
  int npoints = 2500000, nAccepted = 0;
  ...
  // Ici commence la boucle sur les evenements a generer UP
  for (int i=0; i<npoints; i++)
  {
    ...
    beta = acos(cosBeta);
    fmax = sigmaMaxUp * generateur.Rndm();
  ...
  }//fin de la boucle de generation d evenements UP
  
  // Ici commence la boucle sur les evenements a generer DOWN
  for (int i=0; i<npoints; i++)
  {
    // Générer les variables pour construire les 4-vecteurs de le CM
    alpha = generateur.Rndm() * 2*pi; 	//alpha entre 0 et 2pi
    ...
    //x2 = generateur.Rndm() * (1 - 2* x_m) + x_m; //évite les divergences
    beta = acos(cosBeta);
    fmax = sigmaMaxDown * generateur.Rndm();
    // Construisez toute la cinematique dans le referentiel alpha, beta, gamma
    x3 = 2 - x1 - x2;
    ...
    // Inserez la condition de la methode de rejection
    double Sigma = (1-x1)*(1-x2)*dsigma(x1,x2,beta,gamma, -1./3); //pour un quark Down
    if (Sigma > sigmaMaxDown) {
      sigmaMaxDown = Sigma;
      cout << " !!!!!! " << endl;
    }
    if ( Sigma < fmax ) continue;
  ...
  }//fin de la boucle de generation d evenements DOWN
  ...
  cout << "Sigma Max = " << sigmaMaxUp << endl;
  cout << "Eff = " << nAccepted*0.5/npoints << endl;
